...
# ==============================================================================
# Xray (VLESS + REALITY) peer management
# Author: @dna0120
# Version: 5.21.2
...
Commands:
  add <name> [--proto=vision|xhttp]   Add a client
  list                                List clients
  remove <name>                       Revoke a client
  status|check                        Show Xray status
...
case "$COMMAND" in
    add)
        [[ -n "$CLIENT_NAME" ]] || die "Usage: manage_xray.sh add <name> [--proto=vision|xhttp]"
        case "$CLIENT_PROTO" in
            vision|xhttp|1) [[ "$CLIENT_PROTO" == "1" ]] && CLIENT_PROTO="vision" ;;
            2) CLIENT_PROTO="xhttp" ;;
            *) die "Invalid --proto='$CLIENT_PROTO' (vision|xhttp)" ;;
        esac
        mkdir -p "$XRAY_CLIENTS_DIR"
        exec 9>"$XRAY_DIR/.xray_config.lock"
        flock -w 60 9 || die "Could not lock $XRAY_DIR/.xray_config.lock"
        link="$(xray_add_client "$CLIENT_NAME" "$CLIENT_PROTO" | grep '^vless://' | tail -n1)" || die "Failed to add client '$CLIENT_NAME'"
